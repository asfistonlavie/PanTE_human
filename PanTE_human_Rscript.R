...
## note: add your system's directory before "/PanTE_human/" in all the paths used
...
outofAFR_freq <- read.table(file = "/PanTE_human/frequency_distribution/outofAFR/freqoutofAFR.frq", header = T,
                            sep = " ",
                            dec = ".",
                            fill = T) 
...
AFR_freq <- read.table(file = "/PanTE_human/frequency_distribution/AFR/freqAFR.frq", header = T,
                       sep = " ",
                       dec = ".",
                       fill = T) 
...
super_freq <- read.table(file = "/PanTE_human/frequency_distribution/superpop/freqSuper.frq", header = T,
                         sep = " ",
                         dec = ".",
                         fill = T) 
...
TE_anno <- read.table(file = "/PanTE_human/frequency_distribution/RTE_norm_mm90_1alt.vcf", header = F,
                      sep = " ",
                      dec = ".")
...
chr_length <- read.table(file = "/PanTE_human/karyoplote_and_TE_size/chr_lenght_hg28" , header = FALSE,
                         sep = "",
                         dec = ".")
...
ggsave("/PanTE_human/paper_fig/figsupp_TEcountchrlength.png", plot=ggplot2::last_plot())
...
ggsave("/PanTE_human/paper_fig/figsupp_TEsize.png", plot=ggplot2::last_plot())
...
ggsave("/PanTE_human/paper_fig/figsupp_karyotype.png", plot=ggplot2::last_plot())
...
ggsave("/PanTE_human/paper_fig/figsupp_TEcountperchr.png", plot=ggplot2::last_plot())
...
ggsave("/PanTE_human/paper_fig/figsupp_freqspec.png", plot=ggplot2::last_plot())
...
ggsave("/PanTE_human/paper_fig/fig1_freqdist.png", plot=ggplot2::last_plot())
...
annovar_res <- read.table(file = "/PanTE_human/annovar/R_1alt_annovar_out.txt",
                          header=T)
...
region_order <- fct_relevel(TE_newloc$LOC,"Intergenes","Introns","Regulatory","Exons")
...
TE_newloc$Color_Group <- paste0(TE_newloc$Anno, "_", as.numeric(TE_newloc$LOC))
...
p <- ggplot(TE_newloc, aes(x=Anno,y=log10((counts+1)),fill=region_order)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(title = "",
       x = "Genomic region",
       y = "log10 (counts+1)",
       fill = "TE types" ) +
  theme_classic() +
  scale_fill_manual(values = c("#272829","#61677A","#D8D9DA","#FFF6E0"))+
  scale_fill_grey()+
  facet_wrap(~Freq,nrow=1,ncol=5)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1,color = anno_colors, face = "bold"))
g <- ggplot_gtable(ggplot_build(p))
striprt <- which( grepl('strip-r', g$layout$name) | grepl('strip-t', g$layout$name) )
fills <- color_class
k <- 1
...
rec_rate <- read.table(file = "/PanTE_human/recombination_rate/recombAvg.bed", header = F,
                       sep = "",
...
allTE_recrate <- ggplot()+
  geom_boxplot(TE_dataframe_log,mapping=aes(x= freqeuncy,y= mean_recrate,fill=freqeuncy))+
  scale_fill_brewer(palette = "Dark1")+
...
#     axis.title = element_text(size = 12, face = "bold"),
#     axis.text = element_text(size = 10))
# allTE_recrate <-ggplot(TE_dataframe_log,aes(x = freqeuncy, y=mean_recrate,fill=freqeuncy)) + 
#   geom_lv(color='black', size=0.75) + 
#   geom_boxplot(outlier.alpha = 0, coef=0, fill="#00000000") +
#   scale_fill_brewer(palette = "Dark1")+
#   labs(y = "Recombination rate (log10 scaled)",
#        x = "Allele frequency"
#   )+# +
#   # geom_jitter(color="black", size=0.2, alpha=0.2)
#   scale_fill_viridis(discrete = TRUE,option="H")+
...
ggsave("/PanTE_human/paper_fig/fig1_recrate.png", plot=ggplot2::last_plot())
...
background_hg38 <- read.table(file = "/PanTE_human/recombination_rate/background_3713_hg38.bed",header=F)
...
ggsave("/PanTE_human/paper_fig/figsupp_recratebackground.png", plot=ggplot2::last_plot())
...
panic_KD <- read.csv("/PanTE_human/evo_age/Kdistance_filtered.txt",
                     header = F,
                     sep="",
                     dec = ".")
...
ggsave("/PanTE_human/paper_fig/figsupp_evoagediff.png", plot=ggplot2::last_plot())
...
plot_all <- plot_all + labs(x="Divergence age (scaled)",y="Global allele frequency",col="TE type")
...
ggsave("/PanTE_human/paper_fig/figsupp_freqdiv.png", plot=ggplot2::last_plot())

plot_africa <- ggplot()+
  geom_point(TE_div_Ao_neu, mapping=aes(x=scale(div_age),y=FREQ_ALT.a,col=anno_order)) +
  geom_line(TE_div_Ao_neu, mapping=aes(x=scale(exp_age_ancestral),y=FREQ_ALT,col="black"))+
  scale_color_viridis(discrete = TRUE,option = "A")
plot_africa <- plot_africa + scale_colour_manual(values=c(new_colors,"black")) 
plot_africa <- plot_africa + labs(x="Divergence age (scaled)",y="Africa allele frequency",col="TE type")
plot_africa <- plot_africa + theme_linedraw()
plot_africa +  scale_size(trans ="reverse")
  # theme(
  #   plot.title = element_text(size = 16, face = "bold"),
  #   axis.title.x = element_text(size = 14, face = "bold"),
  #   axis.title.y = element_text(size = 14, face = "bold"))
...
plot_outofafrica <- ggplot()+
  geom_point(TE_div_Ao_neu, mapping=aes(x=scale(div_age),y=FREQ_ALT.o,col=anno_order))+
  geom_line(TE_div_Ao_neu, mapping=aes(x=scale(exp_age_ancestral),y=FREQ_ALT,col="black"))
plot_outofafrica <- plot_outofafrica + scale_colour_manual(values=c(new_colors,"black")) 
plot_outofafrica <- plot_outofafrica + labs(x="Divergence age (scaled)",y="Out-of-Africa allele frequency",col="TE type")
plot_outofafrica <- plot_outofafrica + theme_linedraw()
plot_outofafrica +  scale_size(trans ="reverse")
  # theme(
  #   plot.title = element_text(size = 16, face = "bold"),
  #   axis.title.x = element_text(size = 14, face = "bold"),
  #   axis.title.y = element_text(size = 14, face = "bold"))
...
ggsave("/PanTE_human/paper_fig/figsupp_freqdiv_perpop.png", plot=ggplot2::last_plot())

## TE divergence/fst plots

Fst <- read.csv("/PanTE_human/selection_scans/fst/out.weir.fst", header = T,sep = "")   
...
ggsave("/PanTE_human/paper_fig/figsupp_fstdivplot.png", plot=ggplot2::last_plot())
...
population_info <- read.table(file = "/PanTE_human/population_comp/hprc_year1_sample_metadata.txt", header = F,
                              sep = "",
                              dec = ".")

major_TE_filtered_counted <- read.table(file = "/PanTE_human/population_comp/major_TE_stat", header = F,
                                        sep = "",
                                        dec = ".")
...
singletons_TE_filtered_counted <- read.table(file = "/PanTE_human/population_comp/R_singletons_TE_filtered.vcf.singletons", header = F,
                                             sep = "",
                                             dec = ".")
col_order <- c("V2","V1")
...
ggsave("/PanTE_human/paper_fig/fig2_countperind.png", plot=ggplot2::last_plot())
...
TE_indv <- read.table(file = "/PanTE_human/population_comp/TE_counts_1",header = F,sep = "") %>%
  dplyr::select(c(3,9)) %>%
  filter(V3 != "CHM13")
...
ggsave("/PanTE_human/paper_fig/fig2_countperindperpop.png", plot=ggplot2::last_plot())
...
ggsave("/PanTE_human/paper_fig/fig2_countperpopboxplot.png", plot=ggplot2::last_plot())
...
ggsave("/PanTE_human/paper_fig/fig2_freqperpopbox.png", plot=ggplot2::last_plot())
...
pca <- read_table2("/PanTE_human/pca/TE_pca.eigenvec", col_names = FALSE)
eigenval <- scan("/PanTE_human/pca/TE_pca.eigenval")

AFR <- c("HG01891","HG02257","HG02486","HG02559","HG02572","HG03516","HG02622","HG02630","HG02717","HG02886","HG03453","HG03540","HG03579","HG02109","HG02145","HG02723","HG02818","HG03486","NA18906","NA20129","NA21309","HG02055","HG03098")
outofAFR <- c("HG01123","HG01258","HG01358","HG01361","HG00735","HG00741","HG01071","HG01106","HG01175","HG01928","HG01952","HG01978","HG02148","HG00733","HG01109","HG01243","HG00438","HG00621","HG00673","HG02080","HG03492")
pop_info <- read.table(file = "/PanTE_human/frequency_distribution/hprc_year1_sample_metadata.txt", header = F)
...
ggsave("/PanTE_human/paper_fig/fig2_pca.png", plot=ggplot2::last_plot())
...
ggsave("/PanTE_human/paper_fig/figsupp_pca.png", plot=ggplot2::last_plot())
...
directory_path <- "/PanTE_human/selection_scans/ihs_xpehh/outofAFR"
...
directory_path <- "/PanTE_human/selection_scans/ihs_xpehh/AFR"
...
write.table(ihs_AFR_outofAFR_candidate_TE, file = '/PanTE_human/ihs_AFR_outofAFR_candidate_TE.bed', sep = '\t', row.names = F,quote=F)
...
write.table(xpehh_per_0199_new_candidate_TE, file = '/PanTE_human/xpehh_per_0199_new_candidate_TE.bed', sep = '\t', row.names = F,quote=F)

# Ohana demography based selection scans ----------------------------------

ohana <- read.table(file = "/PanTE_human/selection_scans/ohana/scansel.txt", header=T)
pos <-  read.table(file = "/PanTE_human/selection_scans/ohana/TE.map", header=F) %>% dplyr::select(c(4))
all_pos <-  read.table(file = "/PanTE_human/selection_scans/ohana/input_pos_ohana.txt", header=F)
...
write.table(ohana_pos_sig_pos, file = '/PanTE_human/ohana_candidate_TE.bed', sep = '\t', row.names = F,quote=F)
...
write_xlsx(candi_all_2, '/PanTE_human/candi_all.xlsx')
write.table(candi_all_2, '/PanTE_human/candi_all.bed',sep = '\t', row.names = F,quote=F)
...
write_xlsx(list(
                ...
                Sheet18 = candi_all),
           "/PanTE_human/scripts/supp_data_Rdataframes.xlsx")
