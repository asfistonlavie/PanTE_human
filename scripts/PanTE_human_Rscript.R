#### Characterization of Transposable Elements Variations in the Human Pangenome v2
...
new_class_order <- fct_relevel(super_freq_anno$new_class,"Fixed","Polymorphic","Rare")
super_freq_anno_counted$ANNO <- reorder(super_freq_anno_counted$ANNO, -super_freq_anno_counted$NUMBER)
...
super_freq_anno_counted_Q3 <- super_freq_anno_counted %>% filter(CLASS == "Fixed")
super_freq_anno_counted_Q2 <- super_freq_anno_counted %>% filter(CLASS == "Polymorphic")
super_freq_anno_counted_Q1 <- super_freq_anno_counted %>% filter(CLASS == "Rare")

unique(super_freq_anno_counted_Q1$ANNO)
length(unique(super_freq_anno_counted_Q1$ANNO))

A <- ggplot(super_freq_anno_counted_Q3, aes(x="", y=NUMBER,
                                            fill=fct_relevel(ANNO,"LINE/L1","SINE/Alu","SINE/MIR","LTR/ERV","Retroposon/SVA","DNA/misc")))+
  geom_bar(stat="identity", width=1,color = "black") +
  coord_polar("y", start=0,)+
  theme_void()+
...
B <- ggplot(super_freq_anno_counted_Q2, aes(x="", y=NUMBER,
                                            fill=fct_relevel(ANNO,"LINE/L1","LINE/misc","SINE/Alu","SINE/MIR","LTR/ERV","LTR/misc","Retroposon/SVA","DNA/misc")))+
  geom_bar(stat="identity", width=1,color = "black") +
  coord_polar("y", start=0,)+
  theme_void()+
...
C <- ggplot(super_freq_anno_counted_Q1, aes(x="", y=NUMBER,
                                            fill=fct_relevel(ANNO,"LINE/L1","LINE/misc","SINE/Alu","SINE/MIR","LTR/ERV","Retroposon/SVA","DNA/misc")))+
  geom_bar(stat="identity", width=1,color = "black") +
  coord_polar("y", start=0,)+
  theme_void()+
...
combined <- C+B+A & theme(legend.position = "none")
combined + plot_layout(ncol=5, nrow=1, guides = "collect", axis_titles = "collect", tag_level="keep")
...
TE_anno_freq_Gr <- makeGRangesFromDataFrame(df=super_freq_anno,
                                            keep.extra.columns=T,
                                            ignore.strand=T,
                                            seqnames.field="CHR",
                                            start.field="POS",
...
super_freq_anno <- super_freq_anno %>%
  select(c("CHR","POS","ALT","ANNO","END","new_class"))
super_freq_anno_rare <- super_freq_anno %>%
  filter(new_class == "Rare")
super_freq_anno_polymorphic <- super_freq_anno %>%
  filter(new_class == "Polymorphic")
super_freq_anno_fixed <- super_freq_anno %>%
  filter(new_class == "Fixed")

nrow(super_freq_anno_polymorphic)+nrow(super_freq_anno_rare)+nrow(super_freq_anno_fixed)

TE_anno_freq_rare_Gr <- makeGRangesFromDataFrame(df=super_freq_anno_rare,
                                            keep.extra.columns=T,
                                            ignore.strand=T,
                                            seqnames.field="CHR",
                                            start.field="POS",
                                            ...
                                            starts.in.df.are.0based=FALSE)
TE_anno_freq_polymorphic_Gr <- makeGRangesFromDataFrame(df=super_freq_anno_polymorphic,
                                            keep.extra.columns=T,
                                            ignore.strand=T,
                                            seqnames.field="CHR",
                                            start.field="POS",
                                            ...
                                            starts.in.df.are.0based=FALSE)
TE_anno_freq_fixed_Gr <- makeGRangesFromDataFrame(df=super_freq_anno_fixed,
                                            keep.extra.columns=T,
                                            ignore.strand=T,
                                            seqnames.field="CHR",
                                            start.field="POS",
...
TE_size_distTSS_rec_rate$new_class <- lapply(TE_size_distTSS_rec_rate$mean_altfreq, function(x) if(x>=0.95){
...
cor(TE_dataframe$mean_size,TE_dataframe$mean_altfreq, method = "spearman")
cor(TE_dataframe$mean_disttss,TE_dataframe$mean_altfreq, method = "spearman")
cor(TE_dataframe$mean_recrate,TE_dataframe$mean_altfreq, method = "spearman")

cor.test(TE_dataframe$mean_size,TE_dataframe$mean_altfreq, method = "spearman")
cor.test(TE_dataframe$mean_disttss,TE_dataframe$mean_altfreq, method = "spearman")
cor.test(TE_dataframe$mean_recrate,TE_dataframe$mean_altfreq, method = "spearman")

qqnorm(TE_dataframe$mean_altfreq)
qqline(TE_dataframe$mean_altfreq, col = "red")
shapiro.test(TE_dataframe$mean_altfreq)
...
plot_africa <- ggplot()+
  geom_point(data=subset(TE_candi_all,globalsharedlabel=="globalshared"), mapping=aes(x=scale(div_age),y=FREQ_ALT.a,col=candilabel),size=1.5) +
...
plot_outofafrica <- ggplot()+
  geom_point(data=subset(TE_candi_all,globalsharedlabel=="globalshared"), mapping=aes(x=scale(div_age),y=FREQ_ALT.o,col=candilabel),size=1.5) +
  geom_line(data=subset(TE_candi_all,globalsharedlabel=="globalshared"), mapping=aes(x=scale(exp_age_ancestral),y=FREQ_ALT,col="black"))+
  # scale_colour_manual(values=c(new_colors,"black"))+
  scale_color_manual(values=c("black","#EE353E","#F5F5F5"))+
...
sin_pop_v2 <- sin_pop %>%
  ...
  select(c("ind","singletons","super_pop_label"))
...
ggplot(TE_indv_sin_tidy, aes(x = reorder(indv, counts), y = n, fill = interaction(pop, var_types))) +
  geom_col(width = 0.7) +
  scale_y_continuous(breaks = seq(0, 700, by = 50)) +
  scale_fill_manual(
...
## run AnnotSV & VEP on candi_all.bed and extract summary stats
...
ggplot(fst_dt, aes(x = WEIR_AND_COCKERHAM_FST, y = -log10(p_fst))) +
  ...
  geom_point(data = fst_dt[p_fst < 0.05, ], color = "red") +
...
ggplot(merged_ohana_fst_xpehh, aes(x = XPEHH_AFR_outofAFR, y = WEIR_AND_COCKERHAM_FST)) +
  ...
  geom_point(data = merged_ohana_fst_xpehh[
    p_ohana < 0.05 &
      WEIR_AND_COCKERHAM_FST > Fst_percentile_99 &
...
merged_ohana_fst_xpehh_filtered <- as.data.frame(merged_ohana_fst_xpehh %>%
  dplyr::filter(p_ohana < 0.05, 
                WEIR_AND_COCKERHAM_FST > 0.25, 
                LOGPVALUE > 1.30103) %>%
  select(c("chr","pos","WEIR_AND_COCKERHAM_FST","XPEHH_AFR_outofAFR","LOGPVALUE","lrt","p_ohana")))
